...
31 columns are grouped below by role. Multiple compounds from a single paper
...
\col{ground_state_term} & Ground-state term symbol. \\
\col{low_lying_states_eV} & Low-lying state energies (units stated; SOC-free vs.\ SOC-corrected noted). \\
...
\col{ground_state_term} are populated only where the source paper stated
them, so partial coverage is expected and informative rather than a defect.
...
\col{formula}                         & 293 (44\%) \\
\col{ground_state_term}               & 227 (34\%) \\
\col{low_lying_states_eV}             & 172 (26\%) \\
\midrule
...
report a lower-level reference calculation.}
\label{tab:stats}
\end{table}
...
to all 31 fields and linked to its DOI. It is regenerated from the CSV by
\col{site/build_site.py}.
